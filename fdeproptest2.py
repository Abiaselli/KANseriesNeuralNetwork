...
class FDEFractalNoPropNetwork(nn.Module):
    ...
    def forward(self, x, y=None, mode='train'):
        ...
        # Initialize noise
        z = torch.randn(batch_size, self.hidden_dim,self.hidden_dim, device=self.device)
...
def main():
    ...
    model = FDEFractalNoPropNetwork(seq_len, seq_len, vocab_size, T=14, device=device).to(device)
