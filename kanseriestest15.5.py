...
class KANSymbolicDiffusionModel(nn.Module):
    def __init__(self, vocab_size, hidden_dim=128, num_terms=32, degree=3, T=10, device='cpu'):
        ...
        # Diffusion schedule
        self.alpha = torch.linspace(0.99, 0.9, T).to(device)
        self.noise_scale = torch.sqrt(1 - self.alpha)
    ...
    def forward_block(self, x_embed, z, t):
        ...
        stretch_input = torch.cat([x_embed, sym], dim=-1)  # [B, T, hidden_dim + num_terms]
    ...
    def generate_predictions(self, token_ids):
        self.eval()
        with torch.no_grad():
            logits = self.forward(token_ids, mode='inference')
            return torch.argmax(logits, dim=-1)
...
def main():
    ...
    model = KANSymbolicDiffusionModel(vocab_size, hidden_dim=seq_len, num_terms=seq_len, degree=3, T=10, device=device)
