...
class TokenSelector(nn.Module):
    def __init__(self, hidden_dim, token_pool_size):
        ...
        self.reduce = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, token_pool_size)
        )

    def forward(self, hidden):
        # Differentiable output for regression
        probs = F.softmax(self.reduce(hidden), dim=-1)
        token_indices = torch.arange(probs.size(-1), device=probs.device)
        return (probs * token_indices).sum(dim=-1)  # [B]
...
def train_decoder_autoregressive(model, dataset, tokenizer, optimizer, loss_fn, batch_size, seq_len, device):
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for t in range(2, max_len):
            ...
            #token_tracker = ZipfTokenManager(base_pool=range(vocab_size), max_active=5000)
            
            #token_tracker.update(targets.flatten().tolist())


            # Disable autocast
            # with torch.autocast(device_type='cuda', dtype=torch.float16'):
            output = model(src)                    # [B, T]
            print(output.shape)
            targets = tgt_ids                     # [B, T]
            step_loss = F.smooth_l1_loss(output, targets.float())  # Regression-compatible loss
...
def main():
    ...
    model =SymbolicTokenPredictor(vocab_size, seq_length, tokenizer, embed_size, num_terms=3)
