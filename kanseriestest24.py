...
class KANSymbolicDiffusionModel(nn.Module):
    def __init__(self, vocab_size, hidden_dim=128, num_terms=32, degree=3, T=10, device='cpu'):
        super().__init__()
        self.vocab_size = vocab_size
        self.hidden_dim = hidden_dim
        self.num_terms = num_terms
        ...
        # Layers to process input and noisy label
        self.input_embed = nn.Linear(hidden_dim, hidden_dim).double()

        self.stretch = nn.Linear(hidden_dim, hidden_dim).double()
        self.classifier = nn.Linear(hidden_dim, vocab_size).double()
    ...
    def forward_block(self, x_embed, z, t):
        """One symbolic diffusion step"""
        alpha_t = self.alpha[t]

        B, T, H = z.shape

        z_flat = z.view(B * T, H)  # flatten across batch and seq

        sym = self.poly(z_flat[:, 0])  # just one dimension input (scalar symbolic input) if needed
        #print(sym.shape)
        sym = sym.view(B, T, self.num_terms)  # unflatten
        #print(x_embed.shape)
        #print(sym.shape)
        # Stretch symbolic features + x_embed
        stretch_input = torch.einsum('bd,btd->btd', x_embed, sym)


        # Optionally pad to hidden_dim if necessary
        if stretch_input.shape[-1] != self.hidden_dim:
            pad_dim = self.hidden_dim - stretch_input.shape[-1]
            stretch_input = F.pad(stretch_input, (0, pad_dim))

        u = self.stretch(stretch_input)

        z_next = alpha_t * u + (1 - alpha_t) * z + self.noise_scale[t] * torch.randn_like(z)
        return z_next

    def forward(self, token_ids, target_ids=None, mode='train'):
        if token_ids.ndim == 1:
            token_ids = token_ids.unsqueeze(1)
        
        B, T_seq = token_ids.shape

        # Normalize token_ids
        x = token_ids.to(dtype=torch.float64)
        #print(x.shape)
        x_embed = self.input_embed(x)  # [B, T, hidden_dim]

        #print(x_embed.shape)
        #z = torch.randn(B, self.hidden_dim, device=self.device, dtype=torch.float64)
        z = torch.randn(B, T_seq, self.hidden_dim, device=self.device, dtype=torch.float64)

        if mode == 'train':
            losses = []
            target_embed = F.one_hot(target_ids, num_classes=self.vocab_size).to(torch.float64)
            target_embed = target_embed.mean(dim=1)  # [B, vocab_size]
            ...
            for t in range(self.T):
                ...
                loss = F.cross_entropy(pred_logits.view(-1, self.vocab_size), target_ids.view(-1))
                losses.append(loss)
    ...
    def generate_sequence(self, input_prompt, seq_len, max_steps=120):
        ...
        # Normalize token IDs
        x = (input_prompt.float() / self.vocab_size) * 2 - 1
...
def train_symbolic_diffusion_model(model, dataloader, optimizer, device):
        model.train()
        total_loss = 0
        n=0
        for batch_x, batch_y in dataloader:
            ...
            optimizer.zero_grad()
            _, loss = model(batch_x, batch_y, mode='train')  # [B, T, V]
            loss.backward()
            n+=1
            print(f"Iteration {n}, Loss: {loss.item()}")
            optimizer.step()
...
def generate(model, input_text, max_seq_length, device, chunk_size=seq_len, max_generated=120):
    ...
    with torch.no_grad():
        ...
        for _ in range(max_generated):
            output = model(input_ids, input_ids, mode="inference")
            ...
            next_token = torch.argmax(output, dim=-1)[0, -1]  # 🚨 Fix: grab the last token correctly
            ...
            # Append and shift input
            next_token_tensor = next_token.view(1, 1)
            input_ids = torch.cat([input_ids[:, 1:], next_token_tensor], dim=1)
...
def generate_text(model, tokenizer, prompt_text, max_seq_len=64, max_steps=None, device='cuda'):
    model.eval()
    ...
    # Run symbolic diffusion sequence generator
    token_ids = model.generate_sequence(
        input_prompt=prompt_tokens,
        seq_len=max_seq_len,
        max_steps=max_steps if max_steps else model.T
    )

    return tokenizer.decode(token_ids.squeeze(0).tolist(), skip_special_tokens=True)
########################################
# 7. Main Function
########################################
...
def main():
    ...
    model = KANSymbolicDiffusionModel(vocab_size, hidden_dim=seq_len, num_terms=seq_len, degree=5, T=1, device=device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        avg_loss = train_symbolic_diffusion_model(model, dataloader, optimizer, device)
