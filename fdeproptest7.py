...
class FDEFractalNoPropNetwork(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, T=10, device='cpu'):
        ...
        self.W_embed = nn.Parameter(torch.randn(output_dim, hidden_dim))
        ...
        self.alpha = torch.tensor(alpha_cumprod, device=device, dtype=torch.float64)
    ...
    def forward(self, x, y=None, mode='train'):
        ...
        # Initialize noise
        z = torch.randn(batch_size, seq_len,self.hidden_dim, device=self.device)
        ...
        if mode == 'train':
            ...
            for t in range(self.T):
                ...
                pred_embed = z
                pred_embed = pred_embed.reshape(-1)
    ...
    def generate_sequence(self, input_prompt, seq_len, max_steps=120):
        """
        input_prompt: [batch_size, prompt_len] tensor (already tokenized)
        seq_len: int, desired sequence length to generate
        max_steps: number of denoising steps
        """
        self.eval()

        batch_size = input_prompt.size(0)
        device = input_prompt.device

        # Normalize token IDs
        x = (input_prompt.float() / self.output_dim) * 2 - 1
        x_embed = x.to(dtype=torch.float64)
        

        # 🔥 Pad sequence if needed
        prompt_len = x_embed.shape[1]
        if prompt_len < seq_len:
            pad_len = seq_len - prompt_len
            pad_tensor = torch.randn((batch_size, pad_len), device=device, dtype=torch.float64) * 0.01
            x_embed = torch.cat([pad_tensor, x_embed], dim=1)  # [B, seq_len, hidden_dim]
        else:
            x_embed = x_embed[:, :seq_len, :]
        # 🔥 Embed the input
        # Expand token input to seq_len features per token (like positional features)
        x_embed = x_embed.unsqueeze(-1).repeat(1, 1, self.hidden_dim)
        #print(x_embed.shape)
        x_embed = self.input_embed(x_embed)  # [B, prompt_len, hidden_dim]

        #print(x_embed.shape)
       # Initialize noisy z
        z = torch.randn(batch_size, seq_len, self.hidden_dim, device=device, dtype=torch.float64)

        with torch.no_grad():
            for t in range(min(self.T, max_steps)):
                z = self.forward_block(x_embed, z, t)

            # Classify final states
            logits = self.classifier(z)  # [B, seq_len, vocab_size]
            token_ids = torch.argmax(logits, dim=-1)  # [B, seq_len]

        return token_ids

        
    def fde_optimization_step_with_backprop(
        self, population, target_token_ids, tau=1.0, backprop_steps=1, lr=1e-3
    ):
        new_population = []
        fitness_scores = []
        ...
        for alpha, W_embed, params in population:
            self.alpha = alpha.detach().clone().requires_grad_(True)
            self.W_embed = nn.Parameter(W_embed.detach().clone().requires_grad_(True))
            for p, ref in zip(self.parameters(), params):
                p.data = ref.detach().clone().requires_grad_(True)

            logits = self.simulate_curve(self.alpha, self.W_embed, params, seq_len=target_token_ids.size(1), batch_size=target_token_ids.size(0))
            logits = logits.view(-1, self.output_dim)  # [B*T, V]
            targets_flat = target_token_ids.view(-1)   # [B*T]
            ...
            updated_params = [p.detach().clone() for p in self.parameters()]
            updated_W_embed = self.W_embed.detach().clone()
            updated_alpha = self.alpha.detach().clone()
            new_population.append((updated_alpha, updated_W_embed, updated_params))

            if fitness < best_fitness:
                best_fitness = fitness
                best_individual = (updated_alpha, updated_W_embed, updated_params)

        evolved_population = []
        for _ in range(len(population)):
            r1, r2, r3 = torch.randperm(len(new_population))[:3]
            a1, w1, p1 = new_population[r1]
            a2, w2, p2 = new_population[r2]
            a3, w3, p3 = new_population[r3]
            a_mut = a1 + tau * (a2 - a3)
            w_mut = w1 + tau * (w2 - w3)
            p_mut = [p1_i + tau * (p2_i - p3_i) for p1_i, p2_i, p3_i in zip(p1, p2, p3)]
            evolved_population.append((a_mut, w_mut, p_mut))

        return evolved_population, best_individual
    def fde_optimization_step(self, population, target_curve, lr=0.1, tau=0.1):
            ...
            for individual in population:
                ...
                logits = self.simulate_curve(alpha_candidate, W_embed_candidate, params, batch_size=target_curve.size(0))
                #print(logits.shape)
                # Gumbel-Softmax prediction
                gumbel_sample = F.gumbel_softmax(logits, tau=tau, hard=False)


                # Convert target_curve (token IDs) into one-hot vectors
                target_onehot = F.one_hot(target_curve.long(), num_classes=self.output_dim).to(dtype=torch.float64)

                # Fitness: MSE between predicted distribution and target distribution
                fitness = F.mse_loss(gumbel_sample, target_onehot)
                fitness_scores.append(fitness)
            ...
            for _ in range(len(population)):
                ...
                alpha_mut = population[r1][0] + tau * (population[r2][0] - population[r3][0])
                W_embed_mut = population[r1][1] + tau * (population[r2][1] - population[r3][1])
                param_diff = [p2 - p3 for p2, p3 in zip(population[r2][2], population[r3][2])]
                param_mut = [p1 + tau * delta for p1, delta in zip(population[r1][2], param_diff)]

                mutant = (alpha_mut, W_embed_mut, param_mut)

                new_population.append(mutant)

            return new_population, best

    def simulate_curve(self, alpha_candidate, W_embed_candidate, params, seq_len=seq_len, batch_size=10):
        """
        Simulates a forward pass that returns logits over the vocabulary.
        This version assumes each token has an embedding, and we classify them via the shared classifier.
        """

        # [B, T, H] — simulate a batch of embedded noisy inputs
        z_sim = torch.randn(batch_size, seq_len, self.hidden_dim, device=self.device, dtype=torch.float64)
        z_sim = z_sim * 0.1 + (alpha_candidate.mean() * W_embed_candidate.mean())  # inject alpha/W influence

        # Pass through final classifier
        logits = self.classifier(z_sim)  # [B, T, vocab_size]

        # You can flatten over B*T if needed for matching target
        return logits
...
def train_fde_model(model, dataloader, vocab_size, device, population_size=10, steps=100, tau=0.1):
    # Initialize population
    population = [
        (model.alpha.clone(), model.W_embed.clone(), [p.clone() for p in model.parameters()])
    ...
    for step in range(steps):
        ...
        for batch_idx, (src, target) in enumerate(dataloader):
            ...
            # Build a target curve (you might customize this to your problem)
            # For now, assume you take the target embeddings or outputs
            with torch.no_grad():
                target = target.to(dtype=torch.float64)

                target_curve = target.float()  # shape should match simulate_curve output
      
            # Perform evolutionary optimization
            new_population, best_individual = model.fde_optimization_step(
                population,
                target_curve=target_curve,
                tau=tau
            )

            population = new_population  # evolve the population

            # (Optional) Track progress
            logits = model.simulate_curve(*best_individual)

            # Gumbel-Softmax prediction
            gumbel_sample = F.gumbel_softmax(logits, tau=1.0, hard=False)

            # Convert target_curve (token IDs) into one-hot vectors
            target_onehot = F.one_hot(target_curve.long(), num_classes=vocab_size).to(dtype=torch.float64)

            # Fitness: MSE between predicted distribution and target distribution
            best_fitness = F.mse_loss(gumbel_sample, target_onehot)

            total_fitness += best_fitness
            print(f"Step {step} Batch {batch_idx}: Best fitness = {best_fitness:.4f}")

        print(f"=== Evolution Step {step} Completed | Avg Fitness {total_fitness / len(dataloader):.4f} ===")

    # After training: load the best model
    best_alpha, best_W_embed, best_params = best_individual
    model.alpha = best_alpha
    model.W_embed = nn.Parameter(best_W_embed)
    for p_model, p_best in zip(model.parameters(), best_params):
        p_model.data = p_best.data.clone()
...
def generate_text(model, tokenizer, prompt_text, max_seq_len=seq_len, max_steps=None, device='cuda'):
    """
    model: KANSymbolicDiffusionModel
    tokenizer: a HuggingFace tokenizer
    prompt_text: str
    max_seq_len: total sequence length (including prompt)
    max_steps: how many diffusion denoising steps (default = model.T)
    """
    model.eval()
    ...
    # Clip prompt if too long
    if prompt_tokens.shape[1] > max_seq_len:
        prompt_tokens = prompt_tokens[:, :max_seq_len]

    # Run symbolic diffusion to generate full sequence
    generated_token_ids = model.generate_sequence(
        input_prompt=prompt_tokens,
        seq_len=seq_len,
        max_steps=max_steps if max_steps else model.T
    )

    # Decode the output
    generated_text = tokenizer.decode(
        generated_token_ids.squeeze(0).tolist(),  # assuming batch_size=1
        skip_special_tokens=True
    )
    return generated_text
def train_symbolic_diffusion_model(model, dataloader, optimizer, device, epochs=5):
    model.train()
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in dataloader:
            ...
            optimizer.zero_grad()
            logits, loss = model(batch_x, batch_y, mode='train')  # [B, T, V]
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
...
def train_fde_model_hybrid(model, optimizer, dataloader, device, population_size=5, steps=50, tau=0.1, fallback_epochs=2):
    population = [
        (model.alpha.clone(), model.W_embed.clone(), [p.clone() for p in model.parameters()])
        for _ in range(population_size)
    ]
    ...
    for step in range(steps):
        ...
        for batch_idx, (src, target) in enumerate(dataloader):
            src, target = src.to(device), target.to(device)
            new_population, best_individual = model.fde_optimization_step_with_backprop(
                population, target_token_ids=target, tau=tau, backprop_steps=1, lr=1e-3
            )
            logits = model.simulate_curve(*best_individual, seq_len=target.size(1), batch_size=target.size(0))
            logits = logits.view(-1, model.output_dim)
            targets_flat = target.view(-1)
            best_fit = F.cross_entropy(logits, targets_flat)

            if best_fit.item() > 12.0:
                print(f"📉 Fallback triggered at Step {step} Batch {batch_idx} — fitness too high: {best_fit.item():.4f}")
                model.alpha = best_individual[0].detach().clone()
                model.W_embed = nn.Parameter(best_individual[1].detach().clone())
                for p_model, p_best in zip(model.parameters(), best_individual[2]):
                    p_model.data = p_best.detach().clone()
                ...
                for _ in range(fallback_epochs):
                    ...
                    loss.backward()
                    optimizer.step()
                    print(f"   🔧 Gradient fallback loss: {loss.item():.4f}")

                updated_params = [p.clone() for p in model.parameters()]
                best_individual = (model.alpha.clone(), model.W_embed.clone(), updated_params)

            population = new_population
            total_fitness += best_fit.item()

        print(f"=== Evolution Step {step+1} Completed | Avg Fitness: {total_fitness / len(dataloader):.4f} ===")

    model.alpha = best_individual[0]
    model.W_embed = nn.Parameter(best_individual[1])
    for p_model, p_best in zip(model.parameters(), best_individual[2]):
        p_model.data = p_best.data.clone()
def main():
    ...
    generated_text = generate_text(model, tokenizer, prompt, max_seq_len=seq_len, max_steps=None, device=device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_fde_model(model, dataloader, vocab_size, device, population_size=3, steps=5, tau=0.0025)
        
        print(f"Epoch {epoch}/{args.epochs} - Loss: {avg_loss:.4f}")
    train_fde_model_hybrid(model, optimizer, dataloader, device, population_size=3, steps=5, tau=0.001, fallback_epochs=1)
