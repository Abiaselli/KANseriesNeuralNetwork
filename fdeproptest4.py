...
def train_fde_model(model, dataloader, device, population_size=10, steps=100, tau=0.1):
    ...
    for step in range(steps):
        ...
        for batch_idx, (src, target) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                # Assume target is [B, T]
                target_curve = model.classifier(model.input_embed(target.unsqueeze(-1).repeat(1, 1, model.hidden_dim)))
                target_curve = target_curve.sum(dim=1)  # or any compression to match simulate_curve
            ...
            best_fitness = torch.norm(model.simulate_curve(*best_individual) - target_curve).item()
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_fde_model(model, dataloader, device, population_size=10, steps=50, tau=0.1)
