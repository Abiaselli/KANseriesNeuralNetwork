...
tokenizer_path = r"C:\Users\abias\.cursor-tutor\vccdoe\mhlamodel\mhlatest-main\tokenizer"  
...
# Basic symbolic layer using parallel polynomial computation
class SymbolicPolynomialLayer(nn.Module):
    def __init__(self, input_dim=1, num_terms=32, degree=3):
        ...
        # Coefficients for polynomials: shape [num_terms, degree+1]
        self.coeffs = nn.Parameter(torch.randn(num_terms, degree + 1))  # a0, a1, ..., an

        # Route weights per term (forward, backward, cross): [num_terms, 3]
        self.route = nn.Parameter(torch.randn(num_terms, 3))

        # Optional weights for combining terms at the end
        self.term_weight = nn.Parameter(torch.randn(num_terms))

    def forward(self, x):
        """
        x: [B, 1] or [B] input values
        """
        if x.ndim == 1:
            x = x.unsqueeze(-1)  # [B, 1]

        B = x.size(0)
        X = torch.stack([x**i for i in range(self.degree + 1)], dim=-1)  # [B, 1, degree+1]
        poly_vals = torch.einsum('bnd,td->bnt', X.to(device), self.coeffs.to(device))  # [B, 1, num_terms]

        poly_vals = poly_vals.squeeze(1)  # [B, num_terms]
        routed = poly_vals.to(device) * self.term_weight.to(device)  # apply weighting: [B, num_terms]

        return routed  # optionally return full poly_vals if needed

# Mixing logic for combining term outputs
class SymbolicMixer(nn.Module):
    ...
    def forward(self, term_outputs):
        if self.method == 'sum':
            return term_outputs.sum(dim=1)  # [B]
        elif self.method == 'dot':
            return (term_outputs * term_outputs).sum(dim=1)  # dot product variant
        elif self.method == 'cross':
            # Cross-product style pairing (flattened)
            B, T = term_outputs.shape
            expanded = term_outputs.unsqueeze(2) * term_outputs.unsqueeze(1)  # [B, T, T]
            return expanded.sum(dim=(1, 2))  # sum over all pairs
        else:
            raise ValueError("Unsupported mixing method")

# Full symbolic model
class KANSymbolicLayer(nn.Module):
    def __init__(self, num_terms=32, degree=3, mix_method='sum'):
        super().__init__()
        self.poly = SymbolicPolynomialLayer(num_terms=num_terms, degree=degree)
        self.mixer = SymbolicMixer(method=mix_method)

    def forward(self, x):
        terms = self.poly(x)  # [B, num_terms]
        output = self.mixer(terms)  # [B]
        return output

# Small test
class KANSymbolicTokenModel(nn.Module):
    def __init__(self, vocab_size, num_terms=32, degree=3, mix_method='sum'):
        super().__init__()
        self.vocab_size = vocab_size
        self.poly = SymbolicPolynomialLayer(num_terms=num_terms, degree=degree)
        self.mixer = SymbolicMixer(method=mix_method)
        self.out_proj = nn.Linear(1, vocab_size)  # Output: logits over vocab
    ...
    def forward(self, token_ids):
        ...
        x = (token_ids.float() / self.vocab_size) * 2 - 1  # [B, T]

        # Flatten for parallel processing: [B*T]
        x_flat = x.reshape(-1)

        # Symbolic processing: [B*T, num_terms]
        term_outputs = self.poly(x_flat)

        # Combine symbols: [B*T]
        combined = self.mixer(term_outputs)

        # Project to vocab: [B*T, V]
        logits = self.out_proj(combined.unsqueeze(-1))  # [B*T, V]

        # Reshape back to sequence: [B, T, V]
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for t in range(2, max_len):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                logits = model(src)  # Predict next token
                #print(logits.shape)
                #print(tgt_ids.shape)
            ...
            if step_count % 50 == True:
                ...
                #generated_text = generate_3(model,prompt, base_tokenizer, seq_length, device)
...
def main():
    ...
    model = KANSymbolicTokenModel(vocab_size=vocab_size, num_terms=embed_size, degree=num_heads, mix_method='sum')
