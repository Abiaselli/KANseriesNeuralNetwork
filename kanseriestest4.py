...
tokenizer_path = r"C:\Users\Austin\.cursor\ruletransformer-main\mhlatest-main\tokenizer"  
...
class SymbolicTokenPredictor(nn.Module):
    def __init__(self, vocab_size, seq_length, tokenizer, embed_dim=128, num_terms=1):
        ...
        self.terms = nn.ModuleList([SymbolicTerm(func_type=random.choice([
            "sin", "cos", "tan", "poly1", "poly2", "exp", "log"
        ])) for _ in range(num_terms)])
        self.out_proj = nn.Linear(embed_dim, 1)  # now outputs a scalar token index
        self.tokenizer = tokenizer
    ...
    def forward(self, input_ids):
        ...
        for t in range(x.size(1)):
            ...
            for term in self.terms:
                xt = xt + term(xt)
            token_id = self.out_proj(xt).squeeze(-1)  # [B]
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for t in range(2, max_len):
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                ...
                targets = tgt_ids.to(torch.float32)  # Ensure float for MSE
                ...
                output = output[:, :targets.size(1)]  # Match lengths
                step_loss = F.smooth_l1_loss(output, targets)
