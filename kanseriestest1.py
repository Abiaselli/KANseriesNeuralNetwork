...
class SymbolicTokenPredictor(nn.Module):
    def __init__(self, vocab_size, seq_length, tokenizer, embed_dim=128, num_terms=10):
        super().__init__()
    ...
    def forward(self, input_ids):
        ...
        for t in range(x.size(1)):          # autoregressive loop
            ...
            for term in self.terms:
                xt_transformed = xt_transformed + term(xt_transformed)
            out.append(self.output(xt_transformed))  # [B, V]
...
def main():
    ...
    model =SymbolicTokenPredictor(vocab_size, seq_length, tokenizer, embed_size, num_terms=10)
    ...
    generated_text = generate_2(model, prompt, tokenizer, seq_len, device)
