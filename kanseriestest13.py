...
class SymbolicMixer(nn.Module):
    ...
    def forward(self, term_outputs):
        if self.method == 'sum':
            ...
            expanded = term_outputs.unsqueeze(2) * term_outputs.unsqueeze(1)
            return expanded.sum(dim=(1,2))
        else:
            raise ValueError("Unsupported mixing method")

class KANSymbolicTokenModel(nn.Module):
    def __init__(self, vocab_size, num_terms=32, degree=3, mix_method='sum', use_interaction=True):
        super().__init__()
        self.vocab_size = vocab_size
        self.num_terms = num_terms
    ...
    def forward(self, token_ids):
        ...
        mixed = self.mixer(terms)  # [B]

        # terms: [B, T, num_terms]
        B, T, num_terms = terms.shape

        # Flatten batch and sequence first
        terms_flat = terms.view(B * T, num_terms)  # [B*T, num_terms]

        # Now compute term interactions
        interactions = terms_flat.unsqueeze(2) * terms_flat.unsqueeze(1)  # [B*T, num_terms, num_terms]

        # Apply learned interaction weights
        weighted_interactions = interactions * self.interaction_weights  # [B*T, num_terms, num_terms]
        ...
        y = mixed.unsqueeze(-1)  # [B, 1]
        stretched = self.stretch(y)  # [B, 4]
        logits = self.out_proj(stretched)  # [B, vocab_size]
        return logits
